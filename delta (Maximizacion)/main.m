function main( poblacion, cr, f, nombre_archivo, cantidad_de_ejecuciones)
    
    population = poblacion; % 50; %100
    cr = cr; %0.9; %.9
    f = f; %0.3; %.3
    max_generations = 12500;
    
    p.cr = cr;
    p.f = f;
    tic
    for k=1:cantidad_de_ejecuciones
        p = Poblacion( population );
		
		%Guardamos el archivo en cada generación
        ...
        for i=1:max_generations
            ...
            for j=1:length( p.population )
                ...
                p.population( j ) = maxi( [target, trial] );
            end

            % Mostramos la solución
            mejor = maxi( p.population );

            if i == 6000 && mejor.aptitud == 0
                break;
            end
