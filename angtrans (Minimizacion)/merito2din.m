function [fit,viorest,bandera]=merito2din(pn)
%Esta función evalua la funcion de mérito del angulo de transmisión
%del mecanismo de cuatro barras, además evalua las restricciones del problema.
...
if (contx==0)
  ...
  [t,x]=ode45('mod4b_din2',TSPAN,x0,[],pn);  %Solucion del sistema dinamico
...
if (contx==0)
     TRmax=acos((pn(3)^2+pn(4)^2-(pn(1)+pn(2))^2)/(2*pn(3)*pn(4)));
     TRmin=acos((pn(3)^2+pn(4)^2-(pn(1)-pn(2))^2)/(2*pn(3)*pn(4)));
     fmer=(TRmax-(pi/2))^2+(TRmin-(pi/2))^2;
     if nargout==1
         fit=fmer;
     else
         fit=fmer;
